...
ContextConfiguration class SimpleJdbcTemplateTest:AbstractTransactionalTestNGSpringContextTests() {
    ...
    Test fun testQueryForObject() {
        ...
        Assert.assertEquals(bean.id, 1)
        Assert.assertEquals(bean.description, "python")
    }

    Test fun testQueryForInt() {
        Assert.assertEquals(1, template?.queryForInt("select id from test_bean where description = ?", "python"))
    }
}
